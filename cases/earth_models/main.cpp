...
namespace {
    ...
    void print_summary_table(const std::vector<Summary>& summaries) {
        ...
        std::cout << '\n';
        std::cout << std::left << std::setw(name_width) << "Model" << std::right << std::setw(number_width)
                  << "Mass [1e24 kg]" << std::setw(number_width) << "I [1e37 kg m^2]" << std::setw(number_width)
                  << "Mean rho [g/cm^3]" << '\n';
        std::cout << std::string(name_width + 3 * number_width, '-') << '\n';
        std::cout << std::fixed << std::setprecision(6);

        for (const auto& summary : summaries) {
            std::cout << std::left << std::setw(name_width) << summary.name << std::right << std::setw(number_width)
                      << summary.mass_kg / 1.0e24 << std::setw(number_width) << summary.inertia_kg_m2 / 1.0e37
...
int main(int argc, char** argv) {
    try {
        ...
        const auto constant3 = nt::make_layered_constant_3(
            prem, nda::make_view1d(static_cast<const Real_t*>(constant_q3.data()), constant_q3.size()));
        const auto constant5 = nt::make_layered_constant_5(
            prem, nda::make_view1d(static_cast<const Real_t*>(constant_q5.data()), constant_q5.size()));
        const auto scaled3 =
            nt::make_prem_scaled_3(nda::make_view1d(static_cast<const Real_t*>(scaled_q3.data()), scaled_q3.size()));
        const auto scaled5 =
            nt::make_prem_scaled_5(nda::make_view1d(static_cast<const Real_t*>(scaled_q5.data()), scaled_q5.size()));
