...
namespace nt {
    ...
    namespace {
        ...
        std::vector<double> to_vector(const Array& a) {
            return std::vector<double>(a.data(), a.data() + a.extent(0));
        }

        long double cube(long double x) noexcept {
            return x * x * x;
        }
        ...
        //
        // with x = r / R_Earth.
        long double integrate_prem_rho_x2(const EarthProfile& prem,
        ...
        Real_t average_prem_density(const EarthProfile& prem,
                                    Real_t              inner_radius_km,
                                    Real_t              outer_radius_km) {
            const long double a =
                static_cast<long double>(inner_radius_km / earth_radius_km);
            const long double b =
                static_cast<long double>(outer_radius_km / earth_radius_km);
        ...
        LayeredEarth
        make_layered_constant_impl(const EarthProfile&          prem,
                                   nda::View<const Real_t, 1>   factor,
            ...
            for (Index_t i = 0; i < N; ++i) {
                ...
                const Real_t avg_rho =
                    average_prem_density(prem, inner_radius, outer);
        ...
        template <std::size_t N>
        PremScaledEarth
        make_prem_scaled_impl(nda::View<const Real_t, 1>   factor,
                              const std::array<Real_t, N>& outer_radius) {
            assert(factor.extent(0) == N);
        ...
        Index_t layer_index(Index_t                      layers,
                            const std::array<Real_t, 5>& outer_radius,
                            Real_t                       radius_km) noexcept {
        ...
        class PremBackedEarthAtm : public nusquids::EarthAtm {
          public:
            explicit PremBackedEarthAtm(const EarthProfile& prem)
                : EarthAtm(to_vector(prem.radius_fraction),
                           to_vector(prem.density_g_cm3),
                           to_vector(prem.ye)) {
          ...
          protected:
            ...
            Real_t radius_from_track_km(
                const nusquids::GenericTrack& track) const noexcept {
                const Real_t s =
                    (track.GetX() - track.GetInitialX()) / units.km;
                const Real_t L =
                    (track.GetFinalX() - track.GetInitialX()) / units.km;

                const Real_t r2 =
                    radius * radius + s * s - L * s;

                const Real_t r =
                    std::sqrt(std::max(Real_t{0}, r2));
        ...
        class LayeredEarthAtm final : public PremBackedEarthAtm {
          public:
            LayeredEarthAtm(const EarthProfile& prem,
                            const LayeredEarth& earth)
                : PremBackedEarthAtm(prem), earth_(earth) {}

            double density(
                const nusquids::GenericTrack& track) const override {
                const Real_t  r = radius_from_track_km(track);
                const Index_t i =
                    layer_index(earth_.layers,
                                earth_.outer_radius_km, r);
        ...
        class PremScaledEarthAtm final : public PremBackedEarthAtm {
          public:
            PremScaledEarthAtm(const EarthProfile&    prem,
            ...
            double density(
                ...
                return prem_density_at_radius(r) *
                       earth_.density_factor[i];
            }
        ...
        class PerturbedPremEarthAtm final : public PremBackedEarthAtm {
          public:
            PerturbedPremEarthAtm(
                const EarthProfile&        prem,
                const DensityPerturbation& perturbation)
                : PremBackedEarthAtm(prem),
        ...
        std::shared_ptr<nusquids::EarthAtm>
        make_prem_body(const EarthProfile& prem) {
            auto earth = std::make_shared<nusquids::EarthAtm>(
                to_vector(prem.radius_fraction),
                to_vector(prem.density_g_cm3),
                to_vector(prem.ye));
        ...
        void configure_solver(nusquids::nuSQUIDSAtm<>&  nus,
            ...
            nus.Set_EvalThreads(
                static_cast<unsigned int>(options.threads));
        }

        Flux propagate_with_body(
            const Flux&                         initial,
            std::shared_ptr<nusquids::EarthAtm> earth,
            const PropagationOptions&           options) {
            ...
            nusquids::nuSQUIDSAtm<> nus(
                initial.native_coszenith(),
                energy_ev,
                3,
                nusquids::both,
    ...
    Real_t DensityPerturbation::weight(
        Real_t radius_km) const noexcept {
        const Real_t dr = radius_km - center_radius_km;

        if (shape == PerturbationShape::box) {
            const Real_t left =
                center_radius_km - half_width_km;
            const Real_t right =
                center_radius_km + half_width_km;

            if (edge_width_km <= 0.0)
                return radius_km >= left && radius_km <= right
                           ? Real_t{1}
                           : Real_t{0};

            const Real_t w =
                Real_t{0.5} *
                (std::tanh((radius_km - left) /
                           edge_width_km) -
                 std::tanh((radius_km - right) /
                           edge_width_km));
        ...
        if (gaussian_cutoff_sigma > 0.0 &&
            std::abs(z) > gaussian_cutoff_sigma)
            return 0.0;

        return std::exp(-Real_t{0.5} * z * z);
    }

    EarthProfile load_prem(
        const std::filesystem::path& filename) {
        std::ifstream file(filename);

        if (!file)
            throw std::runtime_error(
                "Cannot open PREM file: " +
                filename.string());
    ...
    LayeredEarth make_layered_constant_3(
        const EarthProfile&        prem,
        nda::View<const Real_t, 1> density_factor) {
        return make_layered_constant_impl(
            prem, density_factor, outer_radius_3);
    }

    LayeredEarth make_layered_constant_5(
        const EarthProfile&        prem,
        nda::View<const Real_t, 1> density_factor) {
        return make_layered_constant_impl(
            prem, density_factor, outer_radius_5);
    }

    PremScaledEarth make_prem_scaled_3(
        nda::View<const Real_t, 1> density_factor) {
        return make_prem_scaled_impl(
            density_factor, outer_radius_3);
    }

    PremScaledEarth make_prem_scaled_5(
        nda::View<const Real_t, 1> density_factor) {
        return make_prem_scaled_impl(
            density_factor, outer_radius_5);
    }

    DensityPerturbation make_relative_box_perturbation(
        Real_t center_radius_km,
        Real_t width_km,
    ...
    DensityPerturbation make_absolute_box_perturbation(
        Real_t center_radius_km,
        Real_t width_km,
    ...
    DensityPerturbation make_relative_gaussian_perturbation(
        Real_t center_radius_km,
        Real_t sigma_km,
    ...
    DensityPerturbation make_absolute_gaussian_perturbation(
        Real_t center_radius_km,
        Real_t sigma_km,
    ...
    Flux propagate_flux(
        ...
        return propagate_with_body(
            initial,
            std::make_shared<LayeredEarthAtm>(
                prem, earth),
            options);
    }
    ...
    Flux propagate_flux(
        ...
        return propagate_with_body(
            initial,
            std::make_shared<PremScaledEarthAtm>(
                prem, earth),
            options);
    }
    ...
    Flux propagate_flux(
        ...
        return propagate_with_body(
            initial,
            std::make_shared<PerturbedPremEarthAtm>(
                prem, perturbation),
            options);
    }
